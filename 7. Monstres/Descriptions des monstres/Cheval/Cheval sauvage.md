---
JDR: OSE
Type:
  - core

tags:
  - monster
statblock: inline
aliases:
  - Wild horse
---
# `=this.file.name`


---
```statblock
name: Cheval sauvage
layout: OSE
subtype: "Chevaux légers adaptés pour courir à grande vitesse. Peut survivre en se nourrissant uniquement d’herbe, partout où elle est disponible."
image:
alignement: Neutre
ac: 7 (12)
hp: 9
hit_dice: 2D6
thaco: 18 (+1)
speed: 72m (24m)
jds: [12, 13, 14, 15, 16, (1)]
roll_jds: 1d20
roll-moral: 2d6
moral: 7
xp: "20"
nbr: 0
nbr_dj: 1d10*10
loot:  Aucun
traits:
  - name: Débandade
    desc: "Les troupeaux de 20 individus ou plus peuvent piétiner ceux qui se trouvent sur leur passage. 3-sur-4 chances à chaque round. Bonus de +4 pour toucher les créatures de taille humaine ou plus petites. 10 (1d20) points de dégâts."
  - name: Dressage
    desc: " Le cheval sauvage peut être dressé pour servir de monture (cheval de selle)."
actions:
  - name: 2x sabot
    desc: D20 jet d'attaque, 2 (1d4) de dégâts
source: OSE
```
---
